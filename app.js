...
app.post('/interactions', verifyKeyMiddleware(process.env.PUBLIC_KEY), async function (req, res) {
  ...
  if (type === InteractionType.APPLICATION_COMMAND) {
    ...
    if (name === 'challenge' && id) {
      ...
      const option = req.body.data.options?.[0];
      ...
      const validChoices = ['rock', 'paper', 'scissors', 'cowboy', 'virus', 'computer', 'wumpus'];

      if (!validChoices.includes(objectName.toLowerCase())) {
        return res.send({
          type: InteractionResponseType.CHANNEL_MESSAGE_WITH_SOURCE,
          data: {
            content: 'Please choose a valid move: rock, paper, scissors, cowboy, virus, computer, or wumpus.',
          },
        });
      }
